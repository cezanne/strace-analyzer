...
def write_io_details(data, filename):
    ...
    with open(filename, "w+") as f:
        ...
        for size in sorted(stats.keys()):
            ...
            f.write(",".join(map(str, [size, count, t_tot, t_min, t_max, t_median, size * count / t_tot, size / t_max,
                                       size / t_min])) + "\n")
        ...
        with open(inputfile, 'r') as fh:
            for lineno, line in enumerate(fh):
                ...
                if "execve" in line:
                    ...
                    match = re.search(
                        r'(?P<difftime>[0-9]+\.[0-9]+) execve\((.*)\).*= (?P<fd>-?[0-9]+).*<(?P<open_time>[0-9]+\.[0-9]+)>',
                        line)
                    if match:
                        continue
                elif "dup2(" in line:
                    ...
                    file_access_stats[filename]['open_from'][inputfile] = file_access_stats[filename]['open_from'][
                                                                              inputfile] + 1
                elif "open(" in line:
                    # logging.debug("OPEN:")
                    match = re.search(
                    ...
                    match = re.search(
                        r'(?P<difftime>[0-9]+\.[0-9]+) (?P<func>.*?)\(.*\).*=.*<(?P<time>[0-9]+\.[0-9]+)>', line)
                    if match != None:
                        # logging.debug('{0}'.format(match.groupdict()))
...
def main():
    ...
    all_properties = ['write_time', 'write_count', 'write_size', 'read_time', 'read_count', 'read_size', 'open_time',
